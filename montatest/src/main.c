...
void toggleGPIO( uint32_t  delay, uint16_t GPIO_Pin  );
bool HAL_SPI_MspInit();
...
int main(void) 
{
	...
	/* Select HSI48 Oscillator as PLL source (Phase Lock Loop) Source */
	...
	/* Select PLL as system clock src and configure HCLK & PCLK1 clocks dividers */
	...
	while(1) {
		uint32_t DELAY_TOGGLE_MS = 500u;

		
		//Toggle all LEDs in rotation
		// PC_9 :  LD5 , PC_6 : LD3 , PC_8 : LD4, PC_7 : LD
		toggleGPIO( DELAY_TOGGLE_MS, GPIO_PIN_9);
		toggleGPIO( DELAY_TOGGLE_MS, GPIO_PIN_6);
		toggleGPIO( DELAY_TOGGLE_MS, GPIO_PIN_8);
		toggleGPIO( DELAY_TOGGLE_MS, GPIO_PIN_7);
